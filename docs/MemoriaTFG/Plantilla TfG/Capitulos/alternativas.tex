...
Este cap\'itulo compara la soluci\'on implementada frente a alternativas t\'ecnicas
realistas para el mismo problema.
El objetivo no es defender una \''unica tecnolog\'ia\'', sino justificar por qu\'e,
en el contexto de este TFG (alcance, tiempo, equipo y restricciones de despliegue),
las decisiones adoptadas resultan las m\'as equilibradas.

\section{Criterios de comparaci\'on}

Se han utilizado los siguientes criterios para evaluar opciones:

\begin{itemize}
	\item \textbf{Adecuaci\'on funcional}: capacidad para cubrir requisitos del dominio acad\'emico.
	\item \textbf{Tiempo de implementaci\'on}: esfuerzo necesario para llegar a MVP estable.
	\item \textbf{Mantenibilidad}: claridad estructural y facilidad de evoluci\'on.
	\item \textbf{Coste operativo}: viabilidad en entornos gratuitos o de bajo coste.
	\item \textbf{Seguridad y robustez}: soporte para control de acceso y endurecimiento.
	\item \textbf{Integraci\'on y despliegue}: facilidad de automatizaci\'on CI/CD y despliegue.
\end{itemize}
...
Se han considerado funcionalidades avanzadas (OAuth2 federado y 2FA)
