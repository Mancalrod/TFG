...
Los roles de trabajo utilizados han sido:
\begin{itemize}
\item \textbf{Equipo de desarrollo}: dos autores con responsabilidad compartida en backend,
frontend, pruebas y documentaci\'on t\'ecnica.
\item \textbf{Tutor del TFG}: rol de supervisi\'on funcional y t\'ecnica,
validando hitos, alcance y calidad de entregables.
\item \textbf{Stakeholders acad\'emicos}: referencia de necesidades operativas de uso
(docencia, entrega y evaluaci\'on).
\end{itemize}
...
La gesti\'on del trabajo se ha apoyado en artefactos versionados del propio repositorio
y en herramientas de planificaci\'on integradas con el flujo de desarrollo:

\begin{itemize}
\item \textbf{Product Backlog} y \textbf{Sprint Backlog} para priorizaci\'on y control de alcance.
\item \textbf{GitHub Issues/Projects} para trazabilidad de tareas y decisiones de ejecuci\'on.
\item \textbf{Registro horario por sprint} consolidado en la planificaci\'on del proyecto,
con contraste entre capacidad prevista y dedicaci\'on real.
\end{itemize}
...
El marco de trabajo adoptado combina:
\begin{itemize}
	\item \textbf{Planificaci\'on por objetivos}: cada sprint se orienta a un bloque funcional medible.
	\item \textbf{Control continuo}: seguimiento semanal de avance, calidad y bloqueos.
	\item \textbf{Replanificaci\'on expl\'icita}: adaptaci\'on del calendario cuando cambian supuestos.
\end{itemize}

\subsection{Hip\'otesis de capacidad y calendario base}

Los supuestos de capacidad empleados para el plan temporal han sido los siguientes:
\begin{itemize}
	\item Equipo de desarrollo: \textbf{2 personas}.
	\item Dedicaci\'on estimada: \textbf{30 horas por persona y semana}.
	\item Capacidad total semanal: \textbf{60 horas/semana}.
	\item Duraci\'on de sprint: \textbf{2 semanas}.
	\item Capacidad por sprint: \textbf{120 horas}.
\end{itemize}

Sobre esa base, el proyecto se estructur\'o en \textbf{5 iteraciones} (Sprint 0 + Sprint 1 a Sprint 4),
con un esfuerzo total planificado de \textbf{600 horas}.
...
con asignaci\'on horaria por responsable. El criterio aplicado fue:
\begin{enumerate}
	\item identificar primero las tareas cr\'iticas de MVP;
	\item reservar capacidad para pruebas en cada iteraci\'on;
	\item mantener un \emph{buffer} expl\'icito para incidencias.
\end{enumerate}

En t\'erminos acumulados, la distribuci\'on de esfuerzo se ha orientado a:
\begin{itemize}
	\item construcci\'on de base arquitect\'onica y seguridad en fases tempranas;
	\item desarrollo de funcionalidad de negocio en fases intermedias;
	\item validaci\'on de calidad, documentaci\'on y cierre en fase final.
\end{itemize}
...
Para asegurar gobernanza temporal se definieron hitos de control:
\begin{itemize}
	\item \textbf{H1}: cierre de requisitos y arquitectura (ERS/DAS validados).
	\item \textbf{H2}: disponibilidad de la base t\'ecnica del producto (backend, frontend, seguridad).
	\item \textbf{H3}: primer flujo operativo extremo a extremo (actividad-entrega).
	\item \textbf{H4}: despliegue operativo y pipelines CI/CD estables.
	\item \textbf{H5}: incorporaci\'on del m\'odulo de evaluaci\'on y feedback.
	\item \textbf{H6}: estabilizaci\'on de calidad (pruebas autom\'aticas y mutaci\'on).
	\item \textbf{H7}: cierre de memoria t\'ecnica y validaci\'on documental.
	\item \textbf{H8}: preparaci\'on de defensa y cierre final del proyecto.
\end{itemize}

\subsection{Seguimiento del avance y m\'etricas de control}

El seguimiento temporal se realiz\'o con m\'etricas sencillas pero accionables:
\begin{itemize}
	\item \textbf{Horas planificadas vs. horas completadas} por sprint.
	\item \textbf{Porcentaje acumulado de ejecuci\'on} respecto a las 600 horas totales.
	\item \textbf{Velocidad de entrega} por iteraci\'on (PBIs/tareas completadas).
	\item \textbf{Indicadores de calidad} como condici\'on de cierre (tests en verde, cobertura,
	estabilidad de pipelines).
\end{itemize}

Como referencia de control acumulado:
\begin{itemize}
	\item tras S0: 120 h ejecutadas (20\%);
	\item tras S1: 240 h ejecutadas (40\%);
	\item objetivo tras S2: 360 h (60\%);
	\item objetivo tras S3: 480 h (80\%);
	\item objetivo tras S4: 600 h (100\%).
\end{itemize}

\subsection{Control de desviaciones con valor ganado simplificado}

Para reforzar el control temporal,
se ha aplicado una lectura simplificada de valor ganado,
adaptada a proyectos de alcance acad\'emico.

Se consideran tres magnitudes:
\begin{itemize}
	\item \textbf{PV} (\emph{Planned Value}): horas planificadas acumuladas.
	\item \textbf{EV} (\emph{Earned Value}): horas equivalentes al trabajo realmente completado.
	\item \textbf{AC} (\emph{Actual Cost}): horas reales consumidas.
\end{itemize}
...
de planificaci\'on con trazabilidad de versiones, destacando:
\begin{itemize}
	\item ajuste del modelo de capacidad (de una planificaci\'on inicial m\'as fragmentada
	a una estructura de sprints de 120 h);
	\item recalibraci\'on de fechas de S1-S4 para alinear ejecuci\'on t\'ecnica y calendario acad\'emico;
	\item incorporaci\'on de tareas documentales en S3 para evitar cuello de botella en S4.
\end{itemize}

La replanificaci\'on se adopt\'o con criterio de continuidad: no alter\'o el objetivo global de 600 h,
...
Del seguimiento temporal se extraen tres lecciones principales:

\begin{itemize}
	\item \textbf{Iniciar testing temprano reduce retrabajo}: distribuir pruebas por sprint
	evita acumulaci\'on de defectos al final.
	\item \textbf{Documentar en paralelo reduce riesgo de cierre}: incorporar memoria desde S3
	disminuye el cuello de botella de la fase final.
	\item \textbf{Planificar buffer expl\'icito mejora estabilidad}: la reserva de capacidad
	absorbe incidencias sin romper hitos cr\'iticos.
\end{itemize}
...
Se distinguen dos escenarios para evitar una lectura sesgada:
\begin{itemize}
	\item \textbf{Escenario A (referencia acad\'emica)}: tarifa de 15 EUR/h.
	\item \textbf{Escenario B (referencia profesional junior)}: tarifa de 25 EUR/h.
\end{itemize}

La duraci\'on total considerada es de 600 horas.
...
donde $H_{total}=600$ horas y $T_{hora}$ es la tarifa horaria del escenario.
...
\hline
Escenario A (acad\'emico) & 15 EUR/h & 9\,000 EUR \\
\hline
Escenario B (junior mercado) & 25 EUR/h & 15\,000 EUR \\
\hline
\end{tabular}
\caption{Coste de personal para 600 horas de proyecto}
\label{tab:coste-personal-total}
\end{table}

\subsection{Coste de personal por iteraci\'on}

Dado que cada sprint est\'a planificado en 120 horas, el coste por iteraci\'on es:
...
\hline
\textbf{Total} & \textbf{600} & \textbf{9\,000 EUR} & \textbf{15\,000 EUR} \\
\hline
\end{tabular}
...
\hline
\textbf{Escenario} & \textbf{Sin operaci\'on de pago} & \textbf{Con operaci\'on orientativa} \\
\hline
Escenario A (15 EUR/h) & 9\,000 EUR & 9\,132 EUR \\
\hline
Escenario B (25 EUR/h) & 15\,000 EUR & 15\,132 EUR \\
\hline
\end{tabular}
...
\subsection{An\'alisis de sensibilidad econ\'omica}

Aunque el modelo base usa dos tarifas de referencia,
se incluye un an\'alisis de sensibilidad para escenarios de mercado m\'as exigentes,
manteniendo las 600 horas como constante de alcance.

\begin{table}[ht]
\centering
\begin{tabular}{|P{4.3cm}|P{2.8cm}|P{4.3cm}|}
\hline
\textbf{Escenario de tarifa} & \textbf{Tarifa} & \textbf{Coste total estimado (sin operaci\'on)} \\
\hline
Acad\'emico & 15 EUR/h & 9\,000 EUR \\
\hline
Junior mercado & 25 EUR/h & 15\,000 EUR \\
\hline
Perfil intermedio & 35 EUR/h & 21\,000 EUR \\
\hline
Perfil senior & 45 EUR/h & 27\,000 EUR \\
\hline
\end{tabular}
\caption{Sensibilidad del coste total ante variaci\'on de tarifa horaria}
\label{tab:sensibilidad-costes}
\end{table}
