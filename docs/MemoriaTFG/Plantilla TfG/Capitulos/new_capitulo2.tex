...
La ejecuci\'on del TFG se ha organizado con un enfoque \'agil adaptado a contexto acad\'emico,
...
Los roles de trabajo utilizados han sido el equipo de desarrollo, formado por dos autores con responsabilidad compartida en backend, frontend, pruebas y documentaci\'on t\'ecnica; el tutor del TFG, con funci\'on de supervisi\'on funcional y t\'ecnica para validar hitos, alcance y calidad de entregables; y los stakeholders acad\'emicos, como referencia de necesidades operativas reales en docencia, entrega y evaluaci\'on. Esta combinaci\'on permite equilibrar rigor t\'ecnico con alineaci\'on a uso docente.
...
que prioriza la estabilidad y calidad del software.
...
Para el coste de operaci\'on del sistema se consideran dos perfiles de plataforma diferenciados, de manera an\'aloga a los escenarios de personal: el \textbf{Perfil de Plataforma A} (Acad\'emico/Desarrollo) y el \textbf{Perfil de Plataforma B} (Pago/Producci\'on).

En el \textbf{Perfil A (Desarrollo)}, que representa el escenario real observado durante la ejecuci\'on del TFG, se utilizan de manera predominante planes gratuitos o cr\'editos de estudiante para servicios de integraci\'on, despliegue, base de datos y repositorio, lo que resulta en un coste operativo de 0 EUR. 

Por otro lado, el Perfil B (Producci\'on) representa un entorno real de producci\'on comercial, estimado como coste anual m\'inimo del proveedor PaaS Render para garantizar alta disponibilidad y persistencia sin restricciones de inactividad. La Tabla~\ref{tab:coste-operativo} muestra la comparativa detallada entre ambos perfiles (costes anualizados).

\begin{table}[!htbp]
\centering
\begin{tabular}{|P{5.2cm}|P{3.2cm}|P{3.2cm}|}
\hline
\textbf{Concepto} & \textbf{Coste A (Perfil Acad\'emico)} & \textbf{Coste B (Perfil Producci\'on)} \\
\hline
Hosting backend & 0 EUR & 336 EUR \\
\hline
Base de datos gestionada & 0 EUR & 960 EUR \\
\hline
Servicios auxiliares de almacenamiento/transferencia & 0 EUR & 240 EUR \\
\hline
Dominio y gesti\'on b\'asica & 0 EUR & 48 EUR \\
\hline
\textbf{Total operaci\'on} & \textbf{0 EUR} & \textbf{1\,584 EUR} \\
\hline
\end{tabular}
\caption{Comparativa de costes de infraestructura (anual) según perfil de plataforma}
\label{tab:coste-operativo}
\end{table}

\subsection{Coste total del proyecto y escenarios de sensibilidad}

El coste total del proyecto se modela como la suma de los costes de personal y los costes operativos de infraestructura:
\[
C_{total} = C_{personal} + C_{operacion}
\]

Para ofrecer una visi\'on clara y consistente con el resto del cap\'itulo, se realiza un an\'alisis de sensibilidad cruzando los dos escenarios de tarifas de personal (Escenario A y Escenario B) con los dos perfiles de plataforma analizados (Plataforma A y Plataforma B), manteniendo las 660 horas de esfuerzo como constante.

\begin{table}[!htbp]
\centering
\begin{tabular}{|P{4.2cm}|P{2.5cm}|P{3.2cm}|P{3.2cm}|}
\hline
\textbf{Escenario (Tarifa)} & \textbf{Coste personal} & \textbf{Total con Plataforma A (0 EUR)} & \textbf{Total con Plataforma B (132 EUR)} \\
\hline
Escenario A (Acad\'emico --- 15 EUR/h) & 9\,900 EUR & 9\,900 EUR & 10\,032 EUR \\
\hline
Escenario B (Junior --- 25 EUR/h) & 16\,500 EUR & 16\,500 EUR & 16\,632 EUR \\
\hline
\end{tabular}
\caption{Coste total estimado cruzando tarifas de personal y perfiles de plataforma}
