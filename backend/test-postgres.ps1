...
# ----------------------------------------------------------------
# Crear usuario y BD de la aplicacion si no existen (como superusuario)
# ----------------------------------------------------------------
function Initialize-Database {
    # Intentar conectar como superusuario 'postgres' para crear el usuario tfg y la BD
    $pgSuperPsql = $psqlExe
    if (-not (Test-Path $pgSuperPsql)) { return }

    Write-Host "    Verificando usuario '$DB_USER' en PostgreSQL..." -ForegroundColor Gray
    # Crear usuario tfg si no existe
    $createUser = "DO \$\$ BEGIN CREATE USER $DB_USER WITH PASSWORD '$DB_PASS'; EXCEPTION WHEN duplicate_object THEN NULL; END \$\$;"
    & $pgSuperPsql -h $DB_HOST -p $DB_PORT -U postgres -d postgres -c $createUser 2>$null

    # Crear base de datos tfgdb si no existe
    $dbExists = & $pgSuperPsql -h $DB_HOST -p $DB_PORT -U postgres -d postgres -tAc "SELECT 1 FROM pg_database WHERE datname='$DB_NAME'" 2>$null
    if ($dbExists.Trim() -ne "1") {
        Write-Host "    Creando base de datos '$DB_NAME'..." -ForegroundColor Gray
        & $pgSuperPsql -h $DB_HOST -p $DB_PORT -U postgres -d postgres -c "CREATE DATABASE $DB_NAME OWNER $DB_USER;" 2>$null
    }
    # Garantizar privilegios
    & $pgSuperPsql -h $DB_HOST -p $DB_PORT -U postgres -d postgres -c "GRANT ALL PRIVILEGES ON DATABASE $DB_NAME TO $DB_USER;" 2>$null
    Write-Host "[OK] Usuario '$DB_USER' y BD '$DB_NAME' listos." -ForegroundColor Green
}
...
# --- Crear usuario/BD si es la primera vez ---
Initialize-Database
