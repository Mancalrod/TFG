...
- Usuario y contraseña de PostgreSQL configurados para el proyecto:
  - **Base de datos**: `tfgdb`
  - **Usuario**: `tfg`
  - **Contraseña**: `tfg1234`
  - **Puerto**: `5432`
...
2. **Usar el script de arranque del repositorio**
   Ejecuta `run-postgres.ps1` desde la carpeta backend dentro del proyecto. Este script comprueba si el servidor está
...
Ahora cualquiera que quiera levantar el backend solo necesita `.\run-postgres.ps1` o tener el
...
5. Crea un nuevo servidor en pgAdmin:
   ...
   - En **Connection** completa:
     ...
     - **Password**: `tfg1234`
   - Haz clic en **Save**.
6. Después de conectar, expande `Servers → <tu servidor> → Databases → tfgdb` y navega por los esquemas y tablas.
...
2. Ejecuta:
   ```powershell
   & 'C:\Program Files\PostgreSQL\18\bin\psql.exe' -U tfg -d tfgdb -W
   ```
   Introduce la contraseña (`tfg1234`) cuando se solicite.
3. Comandos útiles dentro de `psql`:
   - `\l` – listar bases de datos
   - `\c tfgdb` – cambiar a la base de la aplicación
   - `\dt` – listar tablas
...
Si al ejecutar `test-postgres.ps1` el backend falla porque no existe la base de datos o el usuario, puedes crearlos manualmente desde `psql`:

```sql
CREATE USER tfg WITH PASSWORD 'tfg1234';
CREATE DATABASE tfgdb OWNER tfg;
GRANT ALL PRIVILEGES ON DATABASE tfgdb TO tfg;
```
...
4. Verifica con `\l` que la base `tfgdb` aparece en el listado.
5. Sal con `\q` y vuelve a lanzar `.\test-postgres.ps1`.
